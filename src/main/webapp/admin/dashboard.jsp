<% if (session.getAttribute("isAdminLogged") == null || !session.getAttribute("isAdminLogged").equals(true)) response.sendRedirect(request.getContextPath()+"/admin");%>
...
<%--            <!-- Profile dropdown -->--%>
<%--&lt;%&ndash;            <div class="ml-3 relative">&ndash;%&gt;--%>
<%--&lt;%&ndash;              <div>&ndash;%&gt;--%>
<%--&lt;%&ndash;                <button type="button" class="bg-white flex text-sm rounded-full focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500" id="user-menu-button" aria-expanded="false" aria-haspopup="true">&ndash;%&gt;--%>
<%--&lt;%&ndash;                  <span class="sr-only">Open user menu</span>&ndash;%&gt;--%>
<%--&lt;%&ndash;                  <img class="h-8 w-8 rounded-full" src="https://images.unsplash.com/photo-1472099645785-5658abf4ff4e?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=facearea&facepad=2&w=256&h=256&q=80" alt="">&ndash;%&gt;--%>
<%--&lt;%&ndash;                </button>&ndash;%&gt;--%>
<%--&lt;%&ndash;              </div>&ndash;%&gt;--%>
          ...
          <div class="px-4 sm:px-6 lg:px-8">
            <div class="sm:flex sm:items-center sm:justify-end">
              <div class="mt-4 sm:mt-0 sm:ml-16 sm:flex-none">
                <a href="${pageContext.request.contextPath}/flights?action=add" class="inline-flex items-center justify-center rounded-md border border-transparent bg-indigo-600 px-4 py-2 text-sm font-medium text-white shadow-sm hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 sm:w-auto">Add New Flight</a>
              </div>
            </div>
            <div class="-mx-4 mt-10 ring-1 ring-gray-300 sm:-mx-6 md:mx-0 md:rounded-lg">
              <table class="min-w-full divide-y divide-gray-300">
                <thead>
                <tr>
                  <th scope="col" class="py-3.5 pl-4 pr-3 text-left text-sm font-semibold text-gray-900 sm:pl-6">Flight N°</th>
                  <th scope="col" class="hidden px-3 py-3.5 text-left text-sm font-semibold text-gray-900 lg:table-cell">Plane Model</th>
                  <th scope="col" class="hidden px-3 py-3.5 text-left text-sm font-semibold text-gray-900 lg:table-cell">AirLine</th>
                  <th scope="col" class="hidden px-3 py-3.5 text-left text-sm font-semibold text-gray-900 lg:table-cell">Departure Airport</th>
                  <th scope="col" class="hidden px-3 py-3.5 text-left text-sm font-semibold text-gray-900 lg:table-cell">Arrival Airport</th>
                  <th scope="col" class="hidden px-3 py-3.5 text-left text-sm font-semibold text-gray-900 lg:table-cell">Departure Date</th>
                  <th scope="col" class="hidden px-3 py-3.5 text-left text-sm font-semibold text-gray-900 lg:table-cell">Arrival Date</th>
                  <th scope="col" class="px-3 py-3.5 text-left text-sm font-semibold text-gray-900">Price</th>
                  <th scope="col" class="hidden px-3 py-3.5 text-left text-sm font-semibold text-gray-900 lg:table-cell">Availability</th>
                ...
                <tbody>
<%--                TODO: foreach every flight--%>
<%--                  <c:if test="${flights != null}">--%>
                    <c:forEach items="${flights}" var="flight">
                        <tr>
                          <td class="relative py-4 pl-4 sm:pl-6 pr-3 text-sm">
                            <div class="font-medium text-gray-900">${flight.id}</div>
                          ...
                          <td class="hidden px-3 py-3.5 text-sm text-gray-500 lg:table-cell">${flight.plane.model}</td>
                          <td class="hidden px-3 py-3.5 text-sm text-gray-500 lg:table-cell">${flight.plane.company.name}</td>
                          <td class="hidden px-3 py-3.5 text-sm text-gray-500 lg:table-cell">${flight.flightPath.departureAirport.name}</td>
                          <td class="hidden px-3 py-3.5 text-sm text-gray-500 lg:table-cell">${flight.flightPath.arriveAirport.name}</td>
                          <td class="hidden px-3 py-3.5 text-sm text-gray-500 lg:table-cell">${flight.flightPath.departureDate}</td>
                          <td class="hidden px-3 py-3.5 text-sm text-gray-500 lg:table-cell">${flight.flightPath.arrivalDate}</td>
                          <td class="px-3 py-3.5 text-sm text-gray-500">
                            <div class="sm:hidden">$${flight.price}</div>
                            <div class="hidden sm:block">$${flight.price}</div>
                          </td><td class="px-3 py-3.5 text-sm text-gray-500">
                            <div class="hidden sm:block">${flight.availability}</div>
                          </td>
